...
data_bxtxn = onp.stack(frames, axis=0) 
...
def lfads_losses(params, key, x_bxt, kl_scale, keep_rate):
  lfads_hps = {'data_dim' : 50, 'ntimesteps' : 50,
            'enc_dim' : 128, 'con_dim' : 128, 'var_min' : 0.001,
            'ic_prior_var' : 0.1, 'ar_mean' : 0.0,
            'ar_autocorrelation_tau' : 1.0,
            'ar_noise_variance' : 0.1,
            'ii_dim' : 1, 'gen_dim' : 128,
            'factors_dim' : 32,
            'l2reg' : 2E-05,
            'batch_size' : 128}
...
def optimize_lfads_core(key, batch_idx_start, num_batches,
  ...
  lfads_opt_hps = {'num_batches' : 200, 'step_size' : 0.05,
                 'decay_steps' : 1, 'decay_factor' : 0.9999,
                 'kl_min' : 0.01, 'kl_max' : 1.0, 'kl_warmup_start' : 500.0,
                 'kl_warmup_end' : 1000.0, 'keep_rate' : 0.98,
                 'max_grad_norm' : 10.0, 'print_every' : 100,
                 'adam_b1' : 0.9, 'adam_b2' : 0.999, 'adam_eps' : 1e-1}
  ...
  def run_update(batch_idx, opt_state):
    ...
    opt_state = update_fun(batch_idx, opt_state, lfads_hps, lfads_opt_hps,next(fkeyg), x_bxt, kl_warmup)
    return opt_state
...
def optimize_lfads(key, init_params, lfads_hps, lfads_opt_hps,train_data, eval_data):
  ...
  def update_w_gc(i, opt_state, lfads_hps, lfads_opt_hps, key, x_bxt,kl_warmup):
  ...
  for oidx in range(num_opt_loops):
    ...
    print(type(dtkey))
    print(type(x_bxt))
    print(type(kl_warmup))
    tlosses = lfads_losses(params, dtkey, x_bxt,
                                     kl_warmup, 1.0)
...
factors = {
    'w':np.array(trained_params_numpy[0])

}

ic_prior = {
    'mean':np.array(trained_params_numpy[1]),
    'logvar':np.array(trained_params_numpy[2])

}

ii_prior = {
    'mean':np.array(trained_params_numpy[3]),
    'logatau':np.array(trained_params_numpy[4]),
    'lognvar':np.array(trained_params_numpy[5])

}

gen = {
    'h0'   : np.array(trained_params_numpy[6]),
    'wRUHX': np.array(trained_params_numpy[7]),
    'wCHX' : np.array(trained_params_numpy[8]),
    'bRU'  : np.array(trained_params_numpy[9]),
    'bC'   : np.array(trained_params_numpy[10])
}
con = {
    'h0'   : np.array(trained_params_numpy[11]),
    'wRUHX': np.array(trained_params_numpy[12]),
    'wCHX' : np.array(trained_params_numpy[13]),
    'bRU'  : np.array(trained_params_numpy[14]),
    'bC'   : np.array(trained_params_numpy[15])
}

gen_ic = {
    'w':np.array(trained_params_numpy[16]),
    'b':np.array(trained_params_numpy[17])

}

con_out = {
    'w':np.array(trained_params_numpy[18]),
    'b':np.array(trained_params_numpy[19])

}
logrates = {
    'w':np.array(trained_params_numpy[20]),
    'b':np.array(trained_params_numpy[21])

}

fwd_rnn = {
    'h0'   : np.array(trained_params_numpy[22]),
    'wRUHX': np.array(trained_params_numpy[23]),
    'wCHX' : np.array(trained_params_numpy[24]),
    'bRU'  : np.array(trained_params_numpy[25]),
    'bC'   : np.array(trained_params_numpy[26])
}
bwd_rnn = {
    'h0'   : np.array(trained_params_numpy[27]),
    'wRUHX': np.array(trained_params_numpy[28]),
    'wCHX' : np.array(trained_params_numpy[29]),
    'bRU'  : np.array(trained_params_numpy[30]),
    'bC'   : np.array(trained_params_numpy[31])
}

ic_enc = {'fwd_rnn':fwd_rnn,'bwd_rnn':bwd_rnn}
init_params = {'ic_enc':ic_enc,
               'gen_ic':gen_ic,
               'ic_prior':ic_prior,
               'con':con,
               'con_out':con_out,
